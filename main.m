...
VANG = zeros(NODES, simLength);
VMAG = zeros(NODES, simLength);
for t=1:simLength
    VANG(:,t) = atand(imag(V(:,t))./real(V(:,t))); % voltage angle in radians
    VMAG(:,t) = abs(V(:,t)); % voltage magnitude
end
...
undirected_netgraph = graph(slist,tlist);
incMatrix = full(incidence(undirected_netgraph));


BRANCHES = length(slist);
C = zeros(NODES,BRANCHES,simLength);
D = zeros(NODES,BRANCHES,simLength);

for t=1:simLength
    for i=1:NODES
        for j=1:BRANCHES
            sindex = find(incMatrix(:,j)==1);
            eindex = find(incMatrix(:,j)==-1);
            C(i,j,t) = abs( incMatrix(i,j) ) * ( VMAG(i,t)^2 - VMAG(sindex,t) * VMAG(eindex,t) * cosd( ( VANG(sindex,t) - VANG(eindex,t) ) ) ) ;
%            C(i,j,t) = abs( incMatrix(i,j) ) * ( VMAG(i,t)^2 - VMAG(sindex,t) * VMAG(eindex,t) * cosd( incMatrix(i,j) * ( VANG(sindex,t) - VANG(eindex,t) ) ) ) ;
            D(i,j,t) = - abs( incMatrix(i,j) ) * VMAG(sindex,t) * VMAG(eindex,t) * sind( ( VANG(sindex,t) - VANG(eindex,t) ) ) ;
%            D(i,j,t) = - abs( incMatrix(i,j) ) * VMAG(sindex,t) * VMAG(eindex,t) * sind( incMatrix(i,j) * ( VANG(sindex,t) - VANG(eindex,t) ) ) ;
        end
    end
end

X = zeros(2*NODES,2*BRANCHES,simLength);
y = zeros(2*NODES,simLength);
...
% check if X(:,:,t) * [g;b] == y(:,t) for a given t ?

Phi = [VMAG;VANG]; % measured voltage phasor
% lets assume we have eps_phi which is the vectorized measurement error of voltage magnitude for all nodes and voltage phase angle for all nodes
% that is Phi = Phi_true + eps_phi ---> Phi_true = Phi - eps_phi

eps_c = zeros(NODES,BRANCHES);
eps_d = zeros(NODES,BRANCHES);
